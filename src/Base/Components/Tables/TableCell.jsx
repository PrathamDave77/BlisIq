...
const actions = cva(
    `flex items-center gap-lg py-lg px-xl border-b-[1px] border-b-border-secondary hover:bg-bg-primary-hover`,
    {
        variants: {
            variant: {
                badge: ""
...
const TableCell = ({ variant, email, username, isAvatar, isFile, isPayment, isBadge, isMultipleBadges }) => {
    return (
        <div className={cn(actions({ variant }))}>
            {isBadge ? <div className="flex py-xxs px-md justify-center items-center gap-md rounded-full border border-success-200 bg-success-50 text-(--color-success-700) content-center font-family-body text-xs font-normal leading-xs">Active</div> : isMultipleBadges ? <><div className="flex py-xxs px-md justify-center items-center gap-md rounded-full border border-success-200 bg-success-50 text-(--color-success-700) content-center font-family-body text-xs font-normal leading-xs">Label</div><div className="flex py-xxs px-md justify-center items-center gap-md rounded-full border border-success-200 bg-success-50 text-(--color-success-700) content-center font-family-body text-xs font-normal leading-xs">Label</div><div className="flex py-xxs px-md justify-center items-center gap-md rounded-full border border-success-200 bg-success-50 text-(--color-success-700) content-center font-family-body text-xs font-normal leading-xs">Label</div></> : <><TableCellActions />
            <div id="Avatar-Label-Group" className="flex flex-1 items-center gap-2 font-family-body leading-sm text-sm ">
                {isAvatar ? <div id="Avatar" className="justify-center items-center flex "><div className="shrink-0 rounded-full border border-solid border-border-secondary-alt bg-[url('../../../public/pkd.jpg')] bg-lightgray bg-position-[50%] bg-cover bg-no-repeat h-10 w-10"></div></div> : isFile ? <div className="flex items-center gap-2"><img src={File_icons} /></div> : isPayment ? <div className="flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" width="28" height="20" viewBox="0 0 28 20" fill="none">
                ...
                <div>
                    <div className="self-stretch line-clamp-1 overflow-ellipsis text-text-primary font-semibold ">{username}</div>
                    <div className="line-clamp-1 text-text-tertiary font-normal">{email}</div>
                </div>
